...
class VAE_v3_4x(nn.Module):
    def __init__(self, up_factor):
        ...
        self.VAE_encoder = nn.Sequential(
            nn.Linear(16*32*64, 2048),
            # nn.ReLU(),
            # nn.Linear(2048, 512),
        )
        self.VAE_decoder = nn.Sequential(
            nn.Linear(1024, 16*32*64),
        )
    ...
    def encode(self, HR_feat):
        ...
        HR_feat = self.init_feat(HR_feat)
        # encoder
        HR_feat = F.interpolate(HR_feat, size=[16, 32], mode='bilinear')
        z_q_mu, z_q_logvar = self.VAE_encoder(HR_feat.view(HR_feat.size(0), -1)).chunk(2, dim=1)
        # z_q_mu = self.bn(z_q_mu)

        return z_q_mu, z_q_logvar

    def decode(self,LR,HR, LR_feat, z_q):
        dec_feat = self.VAE_decoder(z_q).view(LR_feat.size(0), 64, 16, 32)
        ...
        SR = F.interpolate(self.decoder(feat) , size=[416,932], mode='bilinear') + 0.5*F.interpolate(LR, scale_factor=4, mode='bilinear')+0.5*HR


        return SR


    def forward(self ,LR,HR, LR_feat, ref_feat=None, z_q=None):
        # encode
        if z_q is None:
            z_q_mu, z_q_logvar= self.encode(ref_feat)
            z_q = self.reparameterize(z_q_mu, z_q_logvar)
            KL = -0.5 * torch.sum(1 + z_q_logvar - z_q_mu.pow(2) - z_q_logvar.exp())
            # decode
            SR = self.decode(LR, HR,LR_feat, z_q)
...
def rgb2ycbcr(img):
    ...
    return torch.from_numpy(Y).unsqueeze(1).numpy()
    # return Y
